...
## 2026-05-26 第五次更新 — 新文档全面审查发现

来源：`ITOps平台全仓库细粒度代码审查与修正方案.md`（外部审查报告）

### 新文档揭示的更深层问题

本项目经过 2026-05-26 的 API 路径对齐修复后，P0~P1 级**表面问题**已基本处理。但新文档指出更深层的架构问题：

| 层级 | 问题 | 本次处理 | 待处理 |
|------|------|----------|--------|
| **架构** | `api/main.py` 420行承担所有初始化/后台任务/异常处理，违反职责单一 | — | 🔲 P2 |
| **架构** | `modules/business/` 与 `api/routes/` 职责重叠严重 | — | 🔲 P2 |
| **数据模型** | `asset.py` + `device_api.py` 两套设备路由并存，设备主数据模型不统一 | — | 🔲 P2 |
| **自动化** | 后端做触发规则+回滚，前端想做脚本库+任务执行，模型不一致 | — | 🔲 P2 |
| **AI** | `api/routes/ai.py` 只做聊天接口，未嵌入业务上下文（告警/工单/设备） | — | 🔲 P2 |
| **前端** | H1-H29 共 29 个页面存在功能少/不完整/占位问题 | 部分修复 | 🔲 P2 |
| **安全** | 默认 JWT_SECRET、弱口令、凭据明文存储、操作审计缺失 | — | 🔲 P2 |
| **测试** | 缺乏 API 契约测试、E2E 测试、OpenAPI diff 检查 | — | 🔲 P2 |

### 新文档中已确认"已修复"的问题（本项目本次已处理）

以下问题在本次 API 路径对齐中已修复，新文档的"修改方案"无需再实施：

| 新文档章节 | 问题描述 | 当前状态 |
|------------|----------|----------|
| H1 | dev 端口不一致（package.json=5173 vs vite.config.js=3000） | ✅ 已统一为 5173 |
| H4 | `/system/user/list` → `/admin/users` | ✅ 前端已用 `/admin/users` |
| H4 | `/system/role/list` → `/admin/roles` | ✅ 前端已用 `/admin/roles` |
| H5 | `/assets/list` 应为 `/devices` | ✅ 已统一到 `/devices` |
| C7 | 工单处理人硬编码 | ✅ 已改为从 `/admin/users` 加载 |
| C11 | 站内消息接口缺失 | ✅ `/notifications/messages` 已实现 |
| C13 | 登录返回格式不一致 | ✅ 已返回 `access_token` + `token` 双字段 |
| C13 | 修改密码接口 | ✅ `PUT /auth/password` 已实现 |
| B1 | SPA fallback 注册过多 HTTP 方法 | ✅ 已改为仅 GET fallback |
| B1 | 全局异常暴露 traceback | ✅ 已在 main.py 中处理 |
| B1 | 路由 prefix 统一 | ✅ 已统一为 `/api/v1/` |
| F9 | menu/dict 后端服务存在但路由不完整 | ✅ 已通过 admin.py 提供完整路由 |

### 新文档核心建议（不立即实施，仅记录方向）

**第一阶段（1-2周）：修通主链路**
- 统一 `/api/v1/devices` 唯一设备路由，废弃 `asset.py` 设备 CRUD
- 增加网络扫描目标页面（scan target CRUD）
- 修复告警统计、历史指标、TopN 接口
- 增加告警转工单
- 修复默认密钥/弱口令风险

**第二阶段（2-4周）：补齐模块闭环**
- 重写 AI Copilot 和 Analyze 页面
- 重写自动化脚本/任务/执行页面
- 补站内消息接口
- 补菜单/字典接口和前端页面
- 知识库与工单打通
